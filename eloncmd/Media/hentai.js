...
module.exports = async (context) => {
    ...
    if (!text) {
        return m.reply(formatStylishReply("Yo, drop a search query! 🔍 Ex: .hentai hinata"));
    }
    ...
    try {
        ...
        if (!searchData || !searchData.status || !searchData.data || !searchData.data.posts || searchData.data.posts.length === 0) {
            return m.reply(formatStylishReply("No results found for your query! 😢 Try a different search term."));
        }
        ...
        if (!downloadData || !downloadData.status || !downloadData.data || !downloadData.data.gallery || downloadData.data.gallery.length === 0) {
            return m.reply(formatStylishReply("Couldn’t fetch the gallery for this content! 😢 Try again later."));
        }
