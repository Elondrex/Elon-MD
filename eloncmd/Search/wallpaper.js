...
module.exports = async (context) => {
  ...
  if (!text) {
    return m.reply(`Please provide a query. Example: ${prefix}wallpaper anime girl, 5`);
  }
  ...
  try {
    ...
    if (!results.length) {
      await client.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
      return m.reply(`No wallpapers found for "${query}". Try another keyword.`);
    }
    ...
    await client.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
    await m.reply(`Failed to fetch wallpapers. The site may be down or the query is invalid.\nError: ${err.message}`);
  }
};
