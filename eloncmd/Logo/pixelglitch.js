...
module.exports = async (context) => {
    ...
    if (!text) return m.reply("Please type some text to generate the pixel glitch effect.");
    ...
    try {
        ...
        await client.sendMessage(m.chat, {
            image: buffer,
            caption: "🖼️ Pixel Glitch Effect"
        }, { quoted: m });

    } catch (error) {
        await client.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
        m.reply("An error occurred while generating the pixel glitch. Please try again.");
    }
};