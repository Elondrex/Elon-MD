...
module.exports = async (context) => {
    ...
    if (!text) return m.reply("Please type some text to generate the underwater effect.");
    ...
    try {
        ...
        await client.sendMessage(m.chat, {
            image: buffer,
            caption: "🖼️ 3D Underwater Text Effect"
        }, { quoted: m });

    } catch (error) {
        await client.sendMessage(m.chat, { react: { text: '❌', key: m.key } });
        m.reply("An error occurred while generating the underwater text. Please try again.");
    }
};