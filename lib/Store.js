...
if (!fs.existsSync(baseDir)) {
    try {
        ...
        console.error('🄴🄻🄾🄽-🄼🄳 Store Error: Failed to create /app/message_data directory:', e);
    }
}
...
function saveChatData(remoteJid, messageId, chatData) {
    ...
    if (!fs.existsSync(chatDir)) {
        try {
            ...
            console.error('🄴🄻🄾🄽-🄼🄳 Store Error: Failed to create directory:', e);
        }
    }
    ...
    try {
        ...
        console.error('🄴🄻🄾🄽-🄼🄳 Store Error: Failed to save chat data:', error);
    }
}
...
function cleanupOldMessages(maxAgeMs = 24 * 60 * 60 * 1000) {
    try {
        ...
        console.error('🄴🄻🄾🄽-🄼🄳 Store Cleanup Fatal Error:', err);
    }
}
